...
void test_outputs() {
    ...
    ini_glcd();
    clean_glcd();
    printf_fast_f("\x01LCD initialized and cleared.\n");

    // Test Buzzer
    ligar_buzzer(3, 1500);
    printf_fast_f("\x02Buzzer tested.\n");
}

void test_storage() {
    printf_fast_f("\x03Testing RAM SPI...\n");
    ...
    read_data = le_RAM_SPI(0x00);
    if (read_data == test_data) {
        printf_fast_f("\x04RAM SPI test passed.\n");
    } else {
        printf_fast_f("\x04RAM SPI test failed.\n");
    }
}

void test_communication() {
    printf_fast_f("\x05Testing Bluetooth...\n");

    // Test Bluetooth communication
    while (1) {
        if(flag_tecla==1) {
            printf_fast_f("\x06Received: %c\n", tecla);
            flag_tecla = 0;
        }


    }
    printf_fast_f("\x07Bluetooth communication tested.\n");
}

void test_actuator() {
    printf_fast_f("\x01Testing motor de passo...\n");
    controlar_motor_passo(1, 1);
    delay_ms(1000);
    controlar_motor_passo(1, 0);
    printf_fast_f("\x02Motor de passo tested.\n");
}

void test_watchdog() {
    printf_fast_f("\x03Testing Watchdog Timer...\n");
    WDTCN = 0xA5; // Refresh watchdog timer
    printf_fast_f("\x04Watchdog Timer tested.\n");
}
